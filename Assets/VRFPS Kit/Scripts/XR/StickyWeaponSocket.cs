...
namespace VRFPSKit
{
    ...
    {
        public bool dropSocketItemIfGrabbedNewCompatibleItem = false;
        ...
        private void SubscribeToHandEvents()
        {
            ...
            foreach (XRDirectInteractor playerHand in playerObj.GetComponentsInChildren<XRDirectInteractor>())
            {
                ...
                playerHand.selectEntered.AddListener(OnHandGrabNewObject);
            }
        }

        private void OnHandGrabNewObject(SelectEnterEventArgs selectEnterEvent)
        {
            IXRSelectInteractable releasedInteractable = selectEnterEvent.interactableObject;
            
            //If we can't select the interactable that the player released, ignore it
            if (dropSocketItemIfGrabbedNewCompatibleItem)
                if (((_socket.interactionLayers.value & releasedInteractable.interactionLayers.value) != 0))
        ...
        private void OnHandReleaseObject(SelectExitEventArgs selectExitEvent)
        {
            ...
            //If we can't select the interactable that the player released, ignore it
            if (!releasedInteractable.IsSelectableBy(_socket) || !_socket.CanSelect(releasedInteractable)) return;
            if (GetComponentInParent<Damageable>().health < 0) return;//Don't reattach to dead player 
            //Check both interaction masks allows for interaction
            if ((_socket.interactionLayers.value & releasedInteractable.interactionLayers.value) == 0) return;
            
            //Deselect everything in preparation
            _socket.interactionManager.CancelInteractorSelection((IXRSelectInteractor)_socket);
            //Select released object
            _socket.interactionManager.SelectEnterUnconditionally(_socket, releasedInteractable);
        }
    }
}
