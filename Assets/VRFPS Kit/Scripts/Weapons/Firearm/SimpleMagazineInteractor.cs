...
namespace VRFPSKit
{
    ...
    public class SimpleMagazineInteractor : XRImprovedSocketInteractor
    {
        [Space]
        [Header("Magazine Socket Settings")]
        public bool allowGrabMagazine;
        public float minimumEjectDetachDelay = .1f;
        [Space]
        public string[] compatibleMagazineTypes;
        ...
        public void AttachMagazine(Magazine magazine)
        {
            ...
            //Detach interactable
            StartManualInteraction((IXRSelectInteractable) magazine.GetComponent<XRGrabInteractable>());
        }
        ...
        public void EjectMagazine(bool scheduleDetach = false)
        {
            ...
            if(scheduleDetach)
                Invoke(nameof(DetachMagazine), minimumEjectDetachDelay);
        }
        ...
        protected override void OnSelectEntered(SelectEnterEventArgs args)
        {
            ...
            //Check for potentially broken gravity settings
            if(args.interactableObject is XRGrabInteractable grabInteractable && !grabInteractable.forceGravityOnDetach)
                Debug.LogWarning($"Attached magazine {args.interactableObject.transform.gameObject.name} has 'Force Gravity On Detach' disabled, this can lead to magazine floating without gravity when ejecting.");
        }
        ...
        /// <returns></returns>
        private bool MagazineCompatibleFilter(IXRInteractor interactor, IXRInteractable interactable)
        {
            Magazine magazine = interactable.transform.GetComponent<Magazine>();
            if (!magazine) return false;
            if (!compatibleMagazineTypes.Contains(magazine.magazineType)) return false;
        ...
        private static bool MagazineGrabDetachFilter(IXRSelectInteractor interactor, IXRSelectInteractable interactable)
        {
            ...
            return interactor is SimpleMagazineInteractor;
        }
        ...
        protected override void Awake()
        {
            ...
            hoverFilters.Add(new XRHoverFilterDelegate(MagazineCompatibleFilter));

            if (_firearm == null)
                Debug.LogError("Magazine trigger parent components were null");
            
            requirePriorPlayerHold = false; //onlyAcceptHeldInteractables = true will break magazine socket so we disable it
        }
        ...
        protected override void OnValidate()
        {
            ...
            if (requirePriorPlayerHold) {
                Debug.LogWarning("MagazineInteractor will not work with onlyAcceptHeldInteractables enabled, disabling it");
                
                requirePriorPlayerHold = false; 
