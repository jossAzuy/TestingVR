...
namespace VRFPSKit
{
    ...
    public class GrenadePinInteractable : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            //This must run in LateUpdate() as object positions relative to player are corrected in HandPresence.Update()
        ...
        private void UpdatePinJointState()
        {
            if (!pinJoint) return;
            if (!_pinInteractable) return;
            
            bool lockJoint = _pinInteractable.isSelected;
            
            pinJoint.angularXMotion = lockJoint ? ConfigurableJointMotion.Locked : ConfigurableJointMotion.Limited;
            pinJoint.angularYMotion = lockJoint ? ConfigurableJointMotion.Locked : ConfigurableJointMotion.Limited;
            pinJoint.angularZMotion = lockJoint ? ConfigurableJointMotion.Locked : ConfigurableJointMotion.Limited;
        }
